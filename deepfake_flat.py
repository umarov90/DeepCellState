...
def build_frozen(input_size, latent_dim):
    layer_units = [128, 64]
    input_shape = input_size
    drop_rate = 0.5
    inputs = Input(shape=input_shape)
    x = inputs
    # x = Flatten(name="non_frozen_flatten")(x)
    # x = tf.keras.layers.LocallyConnected1D(1, kernel_size=1,
    #                                        activation='linear',
    #                                        kernel_initializer='he_normal',
    #                                        use_bias=False, name="non_frozen_dense")(x)
    x = Dense(978, use_bias=False, name="non_frozen_dense")(x)
    xd = Dropout(0.3, input_shape=(None, 978))(x)
    x = xd
    for f in layer_units:
        x = Dense(f)(x)
        x = LeakyReLU(alpha=0.2)(x)

    x = Dropout(drop_rate, input_shape=(None, layer_units[1]))(x)
    # shape = K.int_shape(x)
    x = Flatten()(x)
    ...
    x = Dropout(drop_rate, input_shape=(None, layer_units[0]))(x)
    # z = Add()([x, xd_input])
    z = tf.keras.layers.Concatenate(axis=-1)([x, xd_input])
    x = Dense(input_size)(z)
    outputs = x
