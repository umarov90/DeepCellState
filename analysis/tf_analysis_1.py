...
for key in ["MCF7"]:
    ...
    for tf in tf_data["MCF7"].keys():
        ...
        pb = np.squeeze(a)
        idx = (-1 * pb).argsort()
        # if np.std(tf_info) > 0.09:
        #     half = int(top_genes_num/2)
        #     top_genes = symbols[idx[:half]].tolist() + symbols[idx[-half:]].tolist()
        # else:
        top_genes = symbols[idx[:top_genes_num]].tolist() # + symbols[idx[-down_genes:]].tolist()
        common_genes = list(set(tf_data[key][tf]).intersection(top_genes))
        res = len(common_genes) / ((top_targets_num / 978) * top_genes_num)
        print(tf + " " + str(res) + " " + str(np.mean(tf_info)) + " " + str(np.std(tf_info))
              + " " + str(np.min(tf_info)) + " " + str(np.max(tf_info)))
...
figure_data = np.stack([figure_vals_num["MCF7"], figure_vals_fold["MCF7"]], axis=0).transpose()
df = pd.DataFrame(data=figure_data, index=tf_chosen, columns=["MCF7_num", "MCF7_fold"])
df.index.name = "TF"
