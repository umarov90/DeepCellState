...
folds = []
for i in range(10):
    folds.append(np.loadtxt("Hodos/our_data/hodos_folds_our_data/"+str(i+1), dtype='str'))
...
for i, fold in enumerate(folds):
    ...
    for cell in cell_types:
        res_hodos_dnpp = pd.read_csv("Hodos/our_data/output/fold_"+str(i+1) + "/" + cell + "_DNPP.csv", delimiter=",", header=None).values
        res_hodos_falrtc = pd.read_csv("Hodos/our_data/output/fold_" + str(i + 1) + "/" + cell + "_FaLRTC.csv", delimiter=",", header=None).values
...
# output the results
with open("Hodos/our_data/perf_hodos_our_data.csv", 'w+') as f:
